...
Note that you can turn on multithreading for efficiently utilizing multiple cores via the macro
MULTITHREADING in Integerpredictor.cpp
for multithreading boost libraries should be installed
this class takes the predictor model's p parameter as the input, can be trained and tested for any
amount of data and training indices
...
public:
    ...
    // function to load data from csv, data can be of any order (0-9 or 9-0, 5-9 and 1-4, etc), this
    // functions also validates for proper data
    // when the digits (left most coloumn) are not in range (0-9) or if the data is not binary, this
    ...
    // find matches count per record for all the rules and store the count in a vector
    // (matches_count_vector)
    void find_matches_count(
        vector<vector<bool> > record, vector<struct rule> rules, vector<int>* matches_count_vector);
    // find the probability vector for all the rules from the total matches count (obtained from
    // method above)
    vector<vector<double> > find_probability_vector(
        vector<vector<int> > total_matches_count, vector<struct rule>* rules);
