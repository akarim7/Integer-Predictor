...
IntegerPredictor::IntegerPredictor(int p_value,string training_filename)
{
    ...
    cout<<"all data loaded with " << all_data_csv.size() << " digits, with each having " << all_data_csv[0].size()<<" data points of " <<all_data_csv[0][1].size() << " bits"<< endl;
...
{
    // if load_digit=999, this function loads the entire data excluding the left most coloumn of the csv
    ...
    while(std::getline(data,line))
    {
       ...
       // check if cell is valid, should contain only 0's and 1's except the leftmost coloumn
        while(std::getline(lineStream,cell,','))
        {
            ...
            if (coloumn==1)
            {
                ...
                // cout<<"coloumn 1 cell is " << cell<< " and patch is " <<patch_for_to_string::to_string(load_digit)  << " and compare result is " << cell.compare(patch_for_to_string::to_string(load_digit))<<endl;
                // for  saving data correspinding to a digit ((!(cell.compare(patch_for_to_string::to_string(load_digit))==0)))
            ...
            {
                if (!((cell.compare("0")==0)||(cell.compare("1")==0))) // the cell values should be either 1 or 0
                {
                    cout<<"not valid" <<endl;
                    breakFlag=true;
                    break;
    ...
    index_vec.push_back(previous_digit);
    cout<<"saving training data of digit " << previous_digit<<endl;
    all_pixels.push_back(pixels_per_digit); // if line = "" then the last digit will not get saved, so saving here
...
int IntegerPredictor::check_rule(vector< vector<bool> > record,IntegerPredictor::rule rule_to_be_checked)
{
    int matches_count=0;
...
bool IntegerPredictor::check_rule_per_record(vector<bool> record_row,struct rule rule_to_be_checked)
{
    for (int j=0;j<rule_to_be_checked.pixels.size();j++)
        {
            //cout<<"rule number " << j <<endl;
...
void IntegerPredictor::find_matches_count(vector< vector<bool> > record,vector<IntegerPredictor::rule> rules,vector<int> *matches_count_vector)
{
...
vector<vector <double> > IntegerPredictor::find_probability_vector(vector<vector <int> > total_matches_count,vector<IntegerPredictor::rule> *rules)
{
    ...
    for (int j=0;j<total_matches_count[0].size();j++)
    {
        for(int i=0;i<total_matches_count.size();i++)
        {
                probability_vector_rule.push_back((double)total_matches_count[i][j]/(double)total_matches_all_data[j]);
        }
...
vector<vector <double> > IntegerPredictor::train()
{
    ...
    #if MULTITHREADING ==1
    cout<<"running training in a multithreaded environment with 10 threads (one per digit)"<<endl;
    std::vector<boost::thread *> z;
    for (int i=0;i<training_index_vec.size();i++)
    {
        z.push_back(new boost::thread(&IntegerPredictor::find_matches_count,this,all_data_csv[i],rules,&total_matches_count[i]));
    }
    ...
    vector< vector <double> > probability_vector=find_probability_vector(total_matches_count,&rules);
...
vector<int> IntegerPredictor::predict(string test_filename, bool use_training_data,int predict_digit)
{
    ...
    {
        cout<<"predict digit not valid, this is the data of the digit used for testing"<<endl;
        return counts_digits;
    }
        ...
        for (int j=0;j<rules.size();j++)
        {
            if (check_rule_per_record(test_data[predict_digit][i],rules[j]))
               // cout<<"rule valid"<<endl;
                final_probability_vector= multiply_vectors(final_probability_vector,probability_vector_training[j]);

        }
        //cout<<"the max element is " << double (*std::max_element(final_probability_vector.begin(), final_probability_vector.end()))<<endl;
        //cout<<"the value at 1 is " << final_probability_vector[1] <<endl;

        int biggest=std::distance(final_probability_vector.begin(), std::max_element(final_probability_vector.begin(), final_probability_vector.end()));
    ...
    cout<< "The accuracy of prediction for digit "<< predict_digit << " is " <<  (float)counts_digits[predict_digit]/(float)test_data[predict_digit].size()<<endl;
    return counts_digits;
